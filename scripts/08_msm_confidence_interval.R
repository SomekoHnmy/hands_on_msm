...
res_se <- cbind(
  ...
  se_cluster_robust_sandwich = se_sandwich[1:3]
)
print(round(res_se, 4))
cat("  → 相関を無視した SE より広くなるのが普通。簡易実装と sandwich パッケージで同等の結果が得られることを確認してください。\n")
cat("    (※ sandwich は小サンプル補正・自由度調整が入るため、わずかに値が異なります。)\n\n")
...
msm_rd_once <- function(d) {
  ...
  num <- glm(metformin_high ~ metformin_high_prev + age_z + smoking + hypertension +
               dyslipidemia + factor(month), family = binomial, data = d)
