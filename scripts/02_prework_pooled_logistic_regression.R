...
risk_diff_from_glm <- function(model, data, use_L) {
  base <- data[!duplicated(data$patient_id),
               c("patient_id","age_z","sex","smoking","hypertension","dyslipidemia")]
  risks <- sapply(c(0, 1), function(a) {
    surv <- rep(1, nrow(base))
    for (k in seq_len(n_int)) {
      nd <- data.frame(base, month = k, metformin_high = a)
      # 【注】hba1c_high は二値(0/1)ですが、ここでは各月の観測平均（割合）を代入することで
      # 周辺化リスクを簡便に近似しています（本来は L=0/1 の各パターンで予測し加重平均する
      # のが厳密ですが、ここではnaiveな回帰モデルの限界を示すための近似計算として行っています）。
...
cat("  → どちらも真値には届かない。調整の仕方で値は動くが、正解ではない。\n\n")
