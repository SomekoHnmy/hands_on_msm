...
L_model_init  <- glm(hba1c_high ~ age_z + smoking + hypertension + dyslipidemia,
...
g_formula <- function(always_high, mc = 50) {
  ...
  base <- dat[!duplicated(dat$patient_id),
              c("age_z","smoking","hypertension","dyslipidemia")]
  ...
  for (k in 1:n_int) {
    ...
    # ステップ2：この月のハザード
    h <- predict(out_model, data.frame(base, metformin_high = A_k, hba1c_high = L_k),
                 type = "response")
...
cat(sprintf("  真の係数版 RD = %+.3f （真値 -0.233 とほぼ一致）\n", rd_true_params))
...
gf_rd_once <- function(d) {
  ...
  om  <- glm(cvd_event ~ metformin_high + hba1c_high +
               age_z + smoking + hypertension + dyslipidemia, binomial, d)
  Li  <- glm(hba1c_high ~ age_z + smoking + hypertension + dyslipidemia,
             binomial, d[d$month == 1, ])
  Lt  <- glm(hba1c_high ~ metformin_high_prev + hba1c_high_prev +
               age_z + smoking + hypertension + dyslipidemia, binomial, d[d$month >= 2, ])
  sim <- function(high) {
    base <- d[!duplicated(d$patient_id), c("age_z","smoking","hypertension","dyslipidemia")]
    ...
    for (k in 1:n_int) {
      ...
      h <- predict(om, data.frame(base, metformin_high=Ak, hba1c_high=Lk), type="response")
...
cat("   MSM（IP weighting）         約 -0.225  （真値近傍）\n")
